...
def train(model, train_dataloader, validation_dataloader):
    ...
    for i in trange(epochs, desc="Epoch"):
      #Save training outputs to check the accuracy calculation
      train_logit_0 = []
      train_logit_1 = []
      train_labels = []
      predictions = []
      ...
      for step, batch in enumerate(train_dataloader):
        ...
        #Check if accuracy calculation >1
        if tmp_eval_accuracy > 1:
            print("==============================================")
            print("Incorrect accuracy found for step {} in epoch {}".format(step, i))
            print("Predictions length {}, Labels length {}".format(len(pred_flat), len(label_ids)))
            print("Calculated accuracy: {}".format(tmp_eval_accuracy))
            print("==============================================")

        tr_acc += tmp_eval_accuracy

        #Save predictions
        train_logit_0.extend(logits[:,0])
        train_logit_1.extend(logits[:,1])
      ...
      epoch_df = pd.DataFrame({"Epoch":i+1, "Logit_0": train_logit_0, "Logit_1":train_logit_1, "Prediction":predictions,
                                "Target":train_labels})
      train_df_list.append(epoch_df)
      # Validation
      ...
      for batch in validation_dataloader:
        ...
        tmp_eval_accuracy, _ = flat_accuracy(logits, label_ids)
    ...
    scratch_path = str(Path(__file__).parent / "../")

    final_train_df = pd.concat(train_df_list)
    final_train_df.to_csv(scratch_path + "/xlnet_acc_test.csv", index = False)

    return train_loss_set, train_acc_set, test_loss_set, test_acc_set

#==============Run hyperparams loop===============
#Mess with batch size
train_dataloader, validation_dataloader = loadData(5)
...
n_warmup = round(total_steps * 0.1)
