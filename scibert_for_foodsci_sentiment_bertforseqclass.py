...
file_id = '1cXkGcnJSRWhLuN-wxRAZ9tG9MSqztx0A'
downloaded = drive.CreateFile({'id': file_id})
downloaded.GetContentFile("mturk_train.csv")

#Load data and apply polarity rule 
mturk_abstracts = pd.read_csv("mturk_train.csv")
...
print('Original length:', len(abstracts[0]))
print("Tokenized length:", len(tokens_list[0]))
print("Nonzero Token IDs length:", torch.sum(torch.ne(input_tensor[0],0)))
...
print("Loading SciBERT sequence classification model...")
...
#Set hyperparams - below are recommendations from paper https://arxiv.org/pdf/1903.10676.pdf
#Optimizer: Adam
...
plt.plot(test_loss, 'b-')
plt.legend(['Training Loss', 'Validation Loss'])
plt.title("Loss: b=32,lr=2e-5,drop=(.5,.1)")
plt.xlabel("Epoch")
plt.ylabel("Loss")
...
plt.plot(test_acc, 'b-')
plt.legend(['Training Accuracy', 'Validation Accuracy'])
plt.title("Accuracy: b=32,lr=2e-5,drop=(.5,.1)")
plt.xlabel("Epoch")
plt.ylabel("Accuracy")
...
for lr in [2e-5, 3e-5, 5e-5]:
  for hidden_dp_prob in [.1, .5, .9]:
    for attention_dp_prob in [.1, .5, .9]:
      ...
      train_dataloader, test_dataloader = create_dataloaders(15, input_tensor, attention_mask_tensor, label_tensor)
