...
mturk_abstracts = pd.read_excel(data_path + "/mturk_train_clean.xlsx")

# Filter out all unrelated abstracts: unrelated >= 3
mturk_abstracts = mturk_abstracts[mturk_abstracts['count_unrelated'] < 3]

print("Total relevant training set size:", len(mturk_abstracts))

# Polarity rule: If (N_positive - N_negative)/5 >= 0.4, then positive
mturk_abstracts['polarity'] = (mturk_abstracts['Average_Score'] >= 0.4).astype(int)

abstracts = mturk_abstracts['AB'].tolist()
labels = mturk_abstracts['polarity'].tolist()

assert len(abstracts) == len(labels)

positive_concentration = sum(mturk_abstracts['polarity'] == 1)/len(mturk_abstracts)

print("The sample is", positive_concentration, "percent positive.")
...
def preProcess(max_len, abstracts, labels):
    ...
    train_inputs, validation_inputs, train_labels, validation_labels, train_masks, validation_masks = train_test_split(input_ids, labels,
                                                                attention_masks, random_state=1, test_size=0.1)
...
def train(model, train_dataloader, validation_dataloader, bsize_iter):
    ...
    for i in trange(epochs, desc="Epoch"):
      ...
      print("Plateau scheduler learning rate: ", scheduler2.get_lr())
      print("Linear schedule learning rate: ", scheduler1.get_lr())
...
plots_path = str(Path(__file__).parent / "../Plots/XLNet_Health_Base")
def plotResults(train_loss_set, train_acc_set, test_loss_set, test_acc_set):
    loss_fname = "/loss_optim{}_nolrdecay.png".format(optim)
    acc_fname = "/accuracy_optim{}_nolrdecay.png".format(optim)
...
model_settings_list = [{'optim': "RMSprop", 'bsize_iter':16, 'maxlen':128},
                        {'optim': "AdamW", 'bsize_iter':16, 'maxlen':128},
                        ]
#Set seed
seed_val = 2
...
for i in range(len(model_settings_list)):
    settings = model_settings_list[i]
    #Model Parameters
    mini_bsize = 8
    bsize_iter = settings['bsize_iter']
    ...
    #Preprocess
    train_inputs, validation_inputs, train_labels, validation_labels, train_masks, validation_masks = preProcess(maxlen, abstracts, labels)
    ...
    epochs = 70 #Solved overfitting with gradient accumulation - now implement early stopping
    total_steps = len(train_dataloader) * epochs
    n_warmup_steps = total_steps * warmup

    #Create learning rate schedulers - linear decay within epoch, exponential decay after loss plateau
    scheduler1 = get_linear_schedule_with_warmup(optimizer,
                                              num_warmup_steps = n_warmup_steps,
                                              num_training_steps = total_steps)
    scheduler2 = ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=3)
    ...
    #torch.save(model.state_dict(), model_path + "/RMSprop_dp0.3_sdp0.1_bsize128.pt")
